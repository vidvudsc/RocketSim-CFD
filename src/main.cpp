...
int main(int, char**)
{
    ...
    GLFWwindow* window = glfwCreateWindow((int)(1500 * main_scale), (int)(900 * main_scale), "RocketSim - 2D Liquid Rocket Flow Lab", nullptr, nullptr);
    ...
    // Setup scaling and RocketSim's engineering UI theme.
    ImGuiStyle& style = ImGui::GetStyle();
    rocket::applyTheme(main_scale);
    ...
    rocket::FlowSolver solver;
    solver.enableGpu(g_PhysicalDevice, g_Device, g_Queue, g_QueueFamily);
    auto flowTexture = std::make_unique<rocket::FlowTexture>(g_PhysicalDevice, g_Device, g_Queue,
                                                             g_QueueFamily, solver.width(), solver.height());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (actions.startBake) {
            solver.setPaused(true);
            bakeManager.start(parameters, actions.bakeSettings);
        }
